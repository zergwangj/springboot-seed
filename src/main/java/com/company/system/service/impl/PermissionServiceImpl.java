...
import com.company.system.service.PermissionService;
...
public class PermissionServiceImpl implements PermissionService {
    ...
    public PermissionDTO findById(long id) {
        Optional<Permission> permission = Optional.ofNullable(permissionMapper.selectById(id));
    ...
    public PermissionDTO create(Permission resources) {
        if(permissionMapper.findByName(resources.getName()) != null){
            throw new EntityExistException(Permission.class,"name",resources.getName());
        }
        permissionMapper.insert(resources);
        return permissionMapStruct.toDto(resources);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void update(Permission resources) {
        Optional<Permission> optionalPermission = Optional.ofNullable(permissionMapper.selectById(resources.getId()));
        ...
        Permission permission1 = permissionMapper.findByName(resources.getName());
